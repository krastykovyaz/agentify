...
def parse_args():
    p = argparse.ArgumentParser(description="Train Gemma LoRA for summarization")
    ...
    p.add_argument(
        "--default-system",
        default="Сделай краткую, точную и фактическую выжимку текста на русском языке без выдумки новых фактов.",
    )
...
def build_prompt(row, default_system):
    ...
    return [
        ...
        {"role": "assistant", "content": ready_text},
    ]
...
def main():
    ...
    # Build chat-template text for Gemma
    df["text"] = df.apply(
        lambda r: tokenizer.apply_chat_template(
            build_prompt(r, args.default_system),
            tokenize=False,
