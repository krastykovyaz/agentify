...
def parse_args():
    p = argparse.ArgumentParser(description="Train Gemma LoRA for extraction")
    ...
    p.add_argument(
        "--default-system",
        default=(
            "Извлеки структуру в JSON строго по полям и без выдумки. "
            "Если значения нет, используй пустую строку или пустой массив."
        ),
    )
...
def build_messages(row: pd.Series, default_system: str):
...
def main():
    ...
    # keep only rows that look like extraction targets if task/domain column exists
    for col in ["task", "domain"]:
        if col in df.columns:
            mask = df[col].astype(str).str.lower().str.contains("extract", regex=False) | df[col].astype(str).str.lower().str.contains("extraction", regex=False)
            if mask.any():
                df = df[mask].copy()
                break
